...
function usernameCheck($username) {
    $connection = new PDO( db_projet_individuel, dbuser, dbpass );
    $checkUser =    $sql = "INSERT INTO user(username, password) VALUES (\"$username\", \"$password_1\")";
    $checkUser->bindParam(':name', $username); 
    $checkUser->execute();
...
<head>
<meta charset="UTF-8">
<title>LOG IN Projet Individuel PHP</title>
<style>
    *{
...
<body >
	<div class="bodyForm">
        ...
        <h1> LOG IN <br/>My Warehouse: Little Poney </h1>
        ...
        if (! ($usernameSuccess ?? true)) {
         ...
         if (! ($passwordSuccess ?? true)) {
              ?>
        	<div>
        		<p>Error Password, min:5 characters!</p>
        	</div>
        	<?php 
          } ?>
            <br/>
